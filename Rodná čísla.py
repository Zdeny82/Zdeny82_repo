...
while len(rc_raw) < 9 or len(rc_raw) > 10:
    rc_raw = input(Fore.RED + "PODMÍNKA 1: nesprávný počet znaků, zadej RČ znovu: " + Fore.RESET)
...
RRRR = uprava_roku_narozeni_na_letopocet(rc_raw)  # pro kratší čísla než 9 musí být ošetřena chyba nebo break
limit_dnů = limit_počtu_dnů_v_měsíci(MM_0, RRRR)

den_bez_nuly = den_bez_nuly(DD)
...
if DD > limit_dnů:
    print(Fore.RED + "PODMÍNKA 2:" + Fore.RESET, f"Nezadali jste platné RČ, měsíc {jmeno_mesice} měl v roce {RRRR} maximálně {limit_dnů} dní")
else:
...
print(" ")
print(f"Datum narození dle zadaného RČ je: {den_bez_nuly}. {jmeno_mesice} {RRRR} ")
print(f"Dle zadaného RC je pohlaví: {pohlaví}")
